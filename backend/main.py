...
def load_data():
    global players_df, centroids_df, scaler, pca, gmm, model_load_error
    ...
    try:
        scaler = joblib.load(models_dir / "scaler_midfield_k5.pkl")
        pca = joblib.load(models_dir / "pca_midfield_k5.pkl")
        gmm = joblib.load(models_dir / "gmm_midfield_k5.pkl")
...
class PredictionRequest(BaseModel):
    attributes: Dict[str, float] = Field(
        ..., description="The 36 football attributes used during model training"
    )
...
def health():
    return {
        "status": "ok",
        "players_loaded": len(players_df) if players_df is not None else 0,
        "models_loaded": all(model is not None for model in (scaler, pca, gmm)),
        "model_load_error": model_load_error,
    }


@app.post("/api/predict")
def predict_player(request: PredictionRequest):
    if not all(model is not None for model in (scaler, pca, gmm)):
        raise HTTPException(status_code=503, detail="Trained models are unavailable")

    missing = [column for column in ATTRIBUTE_COLUMNS if column not in request.attributes]
    if missing:
        raise HTTPException(
            status_code=422,
            detail={"message": "Missing required attributes", "attributes": missing},
        )

    values = np.array([[request.attributes[column] for column in ATTRIBUTE_COLUMNS]])
    scaled_values = scaler.transform(values)
    pca_values = pca.transform(scaled_values)
    probabilities = gmm.predict_proba(pca_values)[0]
    cluster_id = int(np.argmax(probabilities))

    return {
        "role_cluster": cluster_id,
        "cluster_name": CLUSTER_NAMES.get(cluster_id, f"Cluster {cluster_id}"),
        "cluster_probabilities": {
            str(index): round(float(probability), 4)
            for index, probability in enumerate(probabilities)
        },
        "pc1": round(float(pca_values[0, 0]), 4),
        "pc2": round(float(pca_values[0, 1]), 4),
    }
